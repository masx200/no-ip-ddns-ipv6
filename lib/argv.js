var argv = require("yargs")
  .usage(
    "Usage: no-ip -h [hostname] -u [user] -p [password] -t [interval(ms)] -i [customIP] -s"
  )
  .help("help")
  .alias("help", "H")
  .option("hostname", {
  ...
  .example(
    "no-ip -h hello-world.ddns.net -u hello -p s3cr3t",
    "Send a single update request using your current IP address"
  )
  .example(
    "no-ip -h hello-world.ddns.net -u hello -p s3cr3t -i 173.26.2.66",
    "Send a single update request using a custom IP Address"
  )
  .example(
    "no-ip -h hello-world.ddns.net -u hello -p s3cr3t -t 604800000 -s",
    "Start automatic DNS renewal once a week"
  )
  .epilogue(
    "@Author: Rocco Musolino - github.com/roccomuso/no-ip - @Copyright 2017"
  ).argv;
